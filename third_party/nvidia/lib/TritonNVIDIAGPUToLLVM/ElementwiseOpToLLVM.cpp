...
struct SIToFPOpConversion
  ...
  LogicalResult matchAndRewriteInt4ToBf16Conversion(
    ...
    const std::string S4_to_Bf16_sm90 = R"({
      ...
      prmt.b32 $0, r1, r0, 0x1054;
      prmt.b32 $1, r3, r2, 0x1054;
      prmt.b32 $2, r1, r0, 0x3276;
      prmt.b32 $3, r3, r2, 0x3276;
    })";
  ...
  static FailureOr<Value> matchInt4Unpack(Value value) {
    ...
    auto lhsShiftRight = join.getLhs().getDefiningOp<arith::ShRSIOp>();
    if (!lhsShiftRight || !isConst4(lhsShiftRight.getRhs())) return failure();
    auto rhsShiftRight = join.getRhs().getDefiningOp<arith::ShRSIOp>();
    if (!rhsShiftRight || !isConst4(rhsShiftRight.getRhs())) return failure();
    auto lhsShiftLeft = lhsShiftRight.getLhs().getDefiningOp<arith::ShLIOp>();
    if (!lhsShiftLeft || !isConst4(lhsShiftLeft.getRhs())) return failure();
    if (rhsShiftRight.getLhs() != lhsShiftLeft.getLhs()) return failure();
    return rhsShiftRight.getLhs();
  }
