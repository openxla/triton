...
class CudaUtils(object):
    ...
    def __init__(self):
        ...
        PyCUtensorMap = cuda_utils.PyCUtensorMap
...
def make_launcher(signature_types: Sequence[_ArgTypeWithNesting], tensordesc_meta: Sequence[Any]) -> Callable[..., None]:
    ...
    signature_types = _expand_signature(signature_types)
    ...
    def wrapper(grid_dim_x: int, grid_dim_y: int, grid_dim_z: int,
        ...
        cuda_utils.launch(grid_dim_x, grid_dim_y, grid_dim_z, stream, kernel,
                          launch_cooperative_grid, launch_pdl, packed_metadata,
                          hook_args, launch_enter_hook, launch_exit_hook,
...
def wrap_handle_tensordesc(launcher, signature, tensordesc_meta):
    ...
    def inner(*args):
        final_args = list(args[:var_positional_arg])
        tensordesc_idx = 0
        for i, arg in enumerate(args[var_positional_arg:]):
...
class CudaLauncher(object):
    ...
    def __init__(self, src, metadata):
        ...
        self.launch = wrap_handle_tensordesc(launch, signature, tensordesc_meta)
